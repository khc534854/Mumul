...
void UGroupChatUI::SelectGroupChat(class UGroupIconUI* SelectedIcon)
{
    ...
    if (SelectedIcon->bIsChatbotRoom)
    {
    	...
    	if (AMumulPlayerState* PS = Cast<AMumulPlayerState>(GetOwningPlayerState()))
    	{
    		PS->WaitingChannelID = TEXT("Lobby");
    ...
    else
    {
    	...
    	if (ACuteAlienController* PC = Cast<ACuteAlienController>(GetOwningPlayer()))
    	{
    		...
    		{
    			// GameState에서 현재 방의 활성 회의 ID 확인
    			if (AMumulGameState* GS = GetWorld()->GetGameState<AMumulGameState>())
    			{
    				// 현재 선택된 아이콘의 팀 ID 가져오기
    				FString CurrentTeamID = TEXT("");
    				if (SelectedIcon->ChatBlockUI)
    				{
    					CurrentTeamID = SelectedIcon->ChatBlockUI->GetTeamID();
    				}

    				// 회의 중인 방인가?
    				FString MeetingID = GS->GetActiveMeetingID(CurrentTeamID);
    				if (!MeetingID.IsEmpty())
    				{
    					// 모닥불 근처에 있는가?
    					if (PS->bIsNearByCampFire)
    					{
    						if (PC->GetCharacter()->GetCharacterMovement()->MovementMode != MOVE_None)
    						{
    						...
    						if (PC->MeetingComp && PC->MeetingComp->CurrentMeetingSessionID.IsEmpty())
    						{
    							PC->MeetingComp->Client_RequestJoinMeeting(MeetingID);
    						}
    					}
    					else
    					{
    						FTimerHandle BotMsgHandle;
    						GetWorld()->GetTimerManager().SetTimer(BotMsgHandle, [this]()
							{
								if (IsValid(this))
								{
									AddBotChat(TEXT("회의가 진행중입니다. 모닥불 근처로 이동해서 회의에 참여하세요."));
								}
							}, 0.5f, false);
    					}
    				}
		...
		if (FJsonObjectConverter::JsonArrayStringToUStruct(Message, &ChatHistory, 0, 0))
		{
			...
			for (const FTeamChatMessageResponse& Msg : ChatHistory)
			{
				if (Msg.role == "user")
				{
					...
					{
						UMapDataSaveGame* LoadInst = Cast<UMapDataSaveGame>(
...
void UGroupChatUI::OnTextBoxCommitted()
{
	...
	switch (Difficulty)
	{
	case EMumuLeeDifficulty::Beginner: Grade = TEXT("초급");
		break;
	case EMumuLeeDifficulty::Normal: Grade = TEXT("중급");
		break;
	case EMumuLeeDifficulty::Advanced: Grade = TEXT("고급");
		break;
	}
	...
	if (bSuccess)
	{
		...
		if (FJsonObjectConverter::JsonArrayStringToUStruct(Message, &TeamChatList, 0, 0))
		{
			...
			for (const FTeamChatListResponse& TeamChat : TeamChatList)
			{
				...
				{
					FTeamData teamData;
					teamData.UniqueTeamID = TeamChat.teamChatId;
					teamData.TeamName = TeamChat.teamName;
					PS->PS_PlayerTeamList.Add(teamData);
				}
			}
			...
			for (const FTeamChatListResponse& TeamChat : TeamChatList)
			{
				...
				if (ACuteAlienController* PC = Cast<ACuteAlienController>(GetOwningPlayer()))
				{
					PC->ChatComp->Server_AddTeamChatList(TeamChat.teamChatId);
				}
			}
		}
		else
...
void UGroupChatUI::UpdateQuestionButtonState()
{
	...
	if (bIsMeetingChatbotActive)
	{
		...
		QuestionBtn->SetBackgroundColor(FLinearColor(0.12f, 1.f, 0.12f, 1.0f));
	}
	else
	{
		// [OFF 상태] 예: 기본색 (흰색/회색)
		NaNumiIMG->SetBrushFromTexture(NaNumiOffIMG);
		QuestionBtn->SetBackgroundColor(FLinearColor(1.f, 1.f, 1.f, 1.0f));
	}
}
...
void UGroupChatUI::OnBeginnerClicked()
{
	...
	BeginnerBtn->SetBackgroundColor(FLinearColor(0.08f, 1.f, 0.08f, 1.0f));
...
void UGroupChatUI::OnNormalClicked()
{
	...
	IntermediateBtn->SetBackgroundColor(FLinearColor(0.08f, 1.f, 0.08f, 1.0f));
...
void UGroupChatUI::OnAdvancedClicked()
{
	...
	AdvancedBtn->SetBackgroundColor(FLinearColor(0.08f, 1.f, 0.08f, 1.0f));
	BeginnerBtn->SetBackgroundColor(FLinearColor(1.0f, 1.0f, 1.0f, 1.0f));
	IntermediateBtn->SetBackgroundColor(FLinearColor(1.0f, 1.0f, 1.0f, 1.0f));
}
